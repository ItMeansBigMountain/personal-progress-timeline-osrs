# PersonalProgressTimeline Developer Cheat Sheet
...
```bash
cd PersonalProgressTimeline
```

If you cloned the whole HeRmEz workspace with submodules, the path is usually:

```bash
cd HeRmEz/projects/osrs-plugins/PersonalProgressTimeline
```
...
```bash
cd ../../..
git add projects/osrs-plugins/PersonalProgressTimeline
git commit -m "chore: update PersonalProgressTimeline submodule"
git push origin main
```
